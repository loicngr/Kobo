...
describe('createWorkspace()', () => {
  it('crée un workspace et retourne un objet avec les bons champs', async () => {
    ...
    expect(ws.model).toBe('claude-opus-4-8')
...
describe('engine comparison grouping', () => {
  ...
  it('keeps creation order for members created within the same millisecond', async () => {
    ...
    try {
      const created = Array.from({ length: 8 }, (_, i) =>
        createWorkspace({
          ...
          projectPath: '/tmp/p',
          sourceBranch: 'main',
          workingBranch: `feature/task-${i}`,
          comparisonId: 'cmp_same_ms',
        }),
      )
      expect(listComparisonMembers('cmp_same_ms').map((m) => m.id)).toEqual(created.map((w) => w.id))
    } finally {
      vi.useRealTimers()
    }
  })
