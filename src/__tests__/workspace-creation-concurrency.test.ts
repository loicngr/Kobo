...
it('keeps the first checkout when the second concurrent creation rolls back its failed agent', async () => {
  ...
  expect(checkouts).toContain(realpathSync(workspaces[0]!.worktreePath)) // git lists resolved paths
})
...
it('attaches an existing worktree when the project path goes through a symlink', async () => {
  const worktreePath = join(directory, 'external')
  execFileSync('git', ['worktree', 'add', '-b', 'feature/external', worktreePath, 'main'], {
    cwd: repository,
    stdio: 'pipe',
  ...
  const response = await create({ projectPath: alias, worktreePath })
