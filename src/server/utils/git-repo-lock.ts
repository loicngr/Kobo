...
/**
 * Absolute, symlink-resolved path of the git directory shared by every worktree
 * of a repository. Git reports worktree paths resolved, so an unresolved key
 * would split one repository into two locks when it is reached through a link
...
export async function resolveGitCommonDir(repoPath: string): Promise<string> {
  try {
    ...
    return await realpath(stdout.replace(/\n$/, ''))
  } catch {
