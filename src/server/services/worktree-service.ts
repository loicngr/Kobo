...
export async function restoreWorktreeCheckoutUnlocked(
  ...
  try {
    ...
    // The destination is missing, so resolve its parent: git registers resolved paths.
    const canonicalTarget = path.join(await canonicalize(path.dirname(worktreePath)), path.basename(worktreePath))
    // Do not globally prune: another missing checkout may still need its registration.
    if (registrations.some((entry) => path.resolve(entry.path) === canonicalTarget)) {
      throw new WorktreeCheckoutError(
        'path-conflict',
