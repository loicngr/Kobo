...
- **Attention indicators**: workspace cards surface CI failures, review-requested changes, and a conflict-aware **ready-to-merge** badge, plus a one-click **Fix CI** button that spawns a fix workspace automatically. Seven PR/MR and CI transitions can each use the general notification sound, no sound, or an independently selected bundled sound; simultaneous transitions play sequentially.
